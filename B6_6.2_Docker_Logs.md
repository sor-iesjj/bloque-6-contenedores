...
> ### 📌 `B6.6.2` — Registros del servicio con `docker logs`
> - **Bloque 6** (Aislamiento de servicios) · **Fase B6.6** (Monitorización) · **Nivel 1** (Básico) · **RA.05** · **CE.05.c**
> - **🎬 Playlist:** `B6_Contenedores`
...
- **🧩 Problema que resuelve:** en B6.2.3 aprendiste que lo que se escribe dentro **desaparece**. Entonces, ¿dónde quedan los registros? Hoy se responde. **Interpretar las trazas generadas por el propio sistema es el CE.05.c.**
...
> 1. **Paso 0:** léete el ejercicio y ten a mano OBS y tu identificación. **Crea la entrada de apuntes de esta práctica** en Obsidian: fichero `b6-6.2-registros-del-servicio-con-docker-logs.md` dentro de `00_Apuntes/Trimestre_N/B6_Contenedores/`, con la estructura del **Bloque 0 · Fase 0.1.b** y **vacía**. Rellenarla es cosa tuya, después.
...
> ```
>
> [!tip] 🐕 Ponle cara a ese 404
> Acabas de provocar un **404** a propósito. Antes de seguir, míralo:
> ```bash
> curl -s https://http.dog/404.json
> ```
> `http.dog` es una API pública que devuelve **una foto de perro por cada código de estado HTTP**. Suena a broma y es la forma más rápida de que se te queden los cuatro que vas a ver todo el curso:
>
> | Código | Significa | Dónde lo has visto ya |
> | :---: | :--- | :--- |
> | **200** | Todo bien | Cuando `curl http://localhost:8080` funciona |
> | **403** | Prohibido: te identifico, pero no puedes | Al intentar `push` en un repo ajeno (Fase 0.4) |
> | **404** | No existe lo que pides | Justo lo que acabas de hacer |
> | **500** | El servidor ha reventado por dentro | Cuando el contenedor falla, no tú |
>
> Fíjate en la diferencia entre **403 y 404**, que es la que más se confunde: *"existe pero no puedes"* frente a *"no existe"*. Explícala en el vídeo con tus palabras.
