...
> - **Bloque 6** (Aislamiento de servicios) · **Fase B6.4** (Persistencia y permisos) · **Nivel 3** (Avanzado) · **RA4** · **CE 4.a, 4.c**
> - **🎬 Playlist:** `B6_Contenedores`
...
> - **Permiso frente a derecho:** el permiso va en el fichero (lo que se puede hacer); el derecho va en el usuario (lo que el sistema le deja hacer). **CE 4.a.**
...
> 4. **Al terminar:** nombra el vídeo **`B6.4.3 · El choque de UID y GID`** y súbelo a **`B6_Contenedores`** (No listado).
> 5. **Una sola entrega.**
...
>
> Tú no eres root. Compruébalo:
> ```bash
> echo "intento modificar" >> ~/compartida/del_contenedor.txt
> rm ~/compartida/del_contenedor.txt
> ```
> **`Permission denied`** en las dos.
>
> Un contenedor acaba de dejar basura en tu carpeta que tú no puedes tocar. Este es el caso real de la ficha.
...
> [!example] Paso 5 — Recupera el control de tu carpeta
> ```bash
> sudo chown $(id -u):$(id -g) ~/compartida/del_contenedor.txt
> ls -ln ~/compartida
> rm ~/compartida/del_contenedor.txt
> ```
> Ahora sí. Has tenido que usar `sudo` para arreglar el estropicio de un contenedor.
...
> - Has provocado el `Permission denied` **en las dos direcciones**.
...
> 4. Diferencia entre **permiso** y **derecho** (CE 4.a) con un ejemplo de esta práctica.
...
- **Entregable vídeo:** `B6.4.3 · El choque de UID y GID` en `B6_Contenedores`. **Una sola entrega.**
